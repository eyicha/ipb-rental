...
<div id="ownerModal" style="display:none; position:fixed; inset:0; background:rgba(0,0,0,.45); z-index:9999; align-items:center; justify-content:center;"
  ...
  <div style="background:#fff; border-radius:20px; padding:32px; width:min(420px,90vw); box-shadow:0 20px 60px rgba(0,0,0,.2);">
    ...
    <div class="d-flex align-items-center gap-3 mb-4">
      <img src="{{ $item->owner->avatarUrl }}" 
           style="width:64px; height:64px; border-radius:16px; object-fit:cover;" alt="">
      <div>
        <div style="font-weight:700; color:var(--ipb-navy); font-size:18px;">{{ $item->owner->name }}</div>
        @if($item->owner->nim)
        <div style="font-size:13px; color:#7a8fa0;">NIM: {{ $item->owner->nim }}</div>
        @endif
        @if($item->owner->lokasi)
        <div style="font-size:13px; color:#7a8fa0;"><i class="mdi mdi-map-marker-outline"></i> {{ $item->owner->lokasi }}</div>
        @endif
      </div>
    </div>
    ...
    <div class="d-flex gap-3 mb-4">
      <div style="flex:1; text-align:center; background:#f7f7f5; border-radius:12px; padding:12px;">
        <div style="font-size:22px; font-weight:700; color:var(--ipb-navy);">
          {{ $ownerRating > 0 ? $ownerRating : '—' }}
        </div>
      ...
      <div style="flex:1; text-align:center; background:#f7f7f5; border-radius:12px; padding:12px;">
        <div style="font-size:22px; font-weight:700; color:var(--ipb-navy);">
          {{ $item->owner->items()->where('status','aktif')->count() }}
        </div>
...
<div style="display:flex; gap:10px; margin-top:8px;">
  ...
  <a href="{{ route('chat.index', ['with' => $item->owner_id]) }}" 
     style="flex:1; display:flex; align-items:center; justify-content:center; gap:6px; padding:13px; background:var(--ipb-navy); color:#fff; border-radius:12px; font-family:var(--font-body,'DM Sans',sans-serif); font-weight:600; font-size:14px; text-decoration:none;">
    <i class="mdi mdi-chat-outline"></i> Chat
  </a>
  @endif
  @endauth
  <button onclick="document.getElementById('ownerModal').style.display='none'"
    style="flex:1; padding:13px; border-radius:12px; border:1px solid rgba(46,65,86,0.1); background:#fff; font-family:var(--font-body,'DM Sans',sans-serif); font-size:14px; color:#7a8fa0; cursor:pointer;">
    Tutup
  </button>
</div>
...
<div style="background:var(--ipb-cream); border-radius:12px; padding:14px 16px; margin-bottom:16px;" id="priceBreakdown" class="d-none">
  ...
  @if($item->deposit > 0)
  <div class="d-flex justify-content-between mb-1">
    <span style="font-size:13px; color:#7a8fa0;">Deposit <span style="font-size:11px; color:#7a8fa0; font-weight:400;">(jaminan, dikembalikan)</span></span>
  ...
  <div style="font-size:11px; color:#7a8fa0; margin-top:6px; line-height:1.5;">
    * Sudah termasuk deposit Rp {{ number_format($item->deposit, 0, ',', '.') }} yang akan dikembalikan setelah barang kembali dalam kondisi baik.
  </div>
  @endif
</div>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function updatePrice() {
    ...
    const total = sewa + deposit; // ← sudah include deposit
